...
func discoverConfigs() ([]string, error) {
	...
	if len(configPaths) == 0 {
		commonPaths = []string{
			...
			filepath.Join(os.Getenv("HOME"), ".mcp", "config.yaml"),
			filepath.Join(os.Getenv("HOME"), ".mcp", "config.yml"),
			filepath.Join(os.Getenv("HOME"), ".config", "mcp", "config.json"),
			filepath.Join(os.Getenv("HOME"), ".config", "mcp", "config.yaml"),
			filepath.Join(os.Getenv("HOME"), ".config", "mcp", "config.yml"),
	...
	for _, path := range commonPaths {
		if path == "./demos" {
			// Scan directory for JSON and YAML files
