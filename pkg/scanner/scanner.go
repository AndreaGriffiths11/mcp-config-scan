...
var (
	...
	secretPatterns = map[string]*regexp.Regexp{
		...
		// Mistral AI
		"Mistral API Key": regexp.MustCompile(`\b[a-zA-Z0-9]{32}\b`),

		// Cohere
		"Cohere API Key": regexp.MustCompile(`\b[A-Za-z0-9]{40}\b`),

		// Hugging Face
		"Hugging Face Token": regexp.MustCompile(`\bhf_[A-Za-z0-9]{32,}\b`),

		// Replicate
		"Replicate API Token": regexp.MustCompile(`\br8_[A-Za-z0-9]{32,}\b`),

		// Stability AI
		"Stability AI Key": regexp.MustCompile(`\bsk-[A-Za-z0-9]{32,}\b`),

		// ElevenLabs
		"ElevenLabs API Key": regexp.MustCompile(`\b[a-f0-9]{32}\b`),

		// JWT tokens
		"JWT Token": regexp.MustCompile(`\beyJ[A-Za-z0-9_-]+\.eyJ[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+\b`),
...
func isPlaceholder(value string) bool {
	placeholders := []string{
		...
		"hf_example", "r8_example", "sk-example", "eyJexample",
	}
...
func isLikelyRealSecret(secretType, value, location string) bool {
	...
	if strings.Contains(secretType, "Hugging Face Token") && !strings.HasPrefix(value, "hf_") {
		return false
	}

	// Replicate validation
	if strings.Contains(secretType, "Replicate API Token") && !strings.HasPrefix(value, "r8_") {
		return false
	}

	// JWT validation - basic structure check
	if strings.Contains(secretType, "JWT Token") && !strings.HasPrefix(value, "eyJ") {
		return false
	}

	// Mistral and Cohere - length-based validation (they use generic patterns)
	if strings.Contains(secretType, "Mistral API Key") && len(value) != 32 {
		return false
	}

	if strings.Contains(secretType, "Cohere API Key") && len(value) != 40 {
		return false
	}
